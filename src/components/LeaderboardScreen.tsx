...
export function LeaderboardScreen({ game, onBack }: { game: GameState; onBack: () => void }) {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8 animate-fade-in">
      ...
      <div className="surface-card divide-y divide-surface-100 dark:divide-surface-800">
        {allPlayers.map((p, i) => (
          <div key={p.name} className={`flex items-center gap-3 p-3.5 ${p.name === "You" ? "bg-primary-500/5" : ""}`}>
            ...
            <p className="font-bold text-surface-900 dark:text-white text-sm">{p.xp} XP</p>
          </div>
        ))}
      </div>
    </div>
