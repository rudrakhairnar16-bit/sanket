...
export function Sidebar() {
  ...
  const interpreterLinks = [
    { href: "/interpreter", label: "Queue", icon: "🎧" },
    { href: "/interpreter/queue", label: "Requests", icon: "📥" },
    { href: "/interpreter/session", label: "Live Session", icon: "🤝" },
  ];
  ...
  const adminLinks = [
    ...
    { href: "/admin/desks", label: "Desks", icon: "🪑" },
    { href: "/admin/content", label: "Learning Content", icon: "📚" },
    { href: "/admin/signs", label: "Sign Library", icon: "✋" },
  ];

  const isAdmin = ["dept_admin", "org_admin", "state_admin", "national_admin", "super_admin"].includes(user.role);
  const links = user.role === "interpreter" ? interpreterLinks : isAdmin ? (user.role === "national_admin" || user.role === "super_admin" ? [...adminLinks, { href: "/national", label: "National", icon: "🇮🇳" }, { href: "/national/map", label: "National Map", icon: "🗺️" }, { href: "/national/states", label: "States", icon: "📍" }, { href: "/national/impact", label: "Impact", icon: "📈" }] : adminLinks) : clerkLinks;
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 min-h-[calc(100vh-4rem)] glass border-r border-white/10 p-4" role="navigation" aria-label="Sidebar navigation">
      <div className="mb-4 px-3">
        <p className="text-xs text-white/40 uppercase tracking-wider font-medium">{user.role === "interpreter" ? "Interpreter Desk" : isAdmin ? "Admin Panel" : "Quick Nav"}</p>
      </div>
