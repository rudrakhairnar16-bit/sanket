...
import { MUNICIPAL_SIGNS, SIGN_MAP } from "@/data/municipal-signs";
...
export default function LiveInterpreter() {
  ...
  useEffect(() => {
    ...
    if (saved && saved.length > 20) {
      ...
      setCalibrated(classifier.getSignCount() > 0);
    }
  }, []);
    ...
    <div className="flex flex-col h-screen bg-gray-950 text-white">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-900 border-b border-gray-800 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold">Live Interpreter</span>
          {calibrated && (
            <span className="text-xs bg-green-900/50 text-green-400 px-2 py-0.5 rounded-full">
              {classifier.getSignCount()} signs loaded
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        {/* Camera + Overlay */}
        <div className="lg:w-1/2 relative bg-gray-900 flex items-center justify-center min-h-[240px]">
          ...
          {!calibrated && status === "running" && (
            <div className="absolute inset-0 flex items-center justify-center bg-gray-950/70 pointer-events-none">
              <div className="text-center max-w-xs">
                <span className="text-4xl block mb-2">⚙️</span>
                <p className="text-sm text-gray-300">No sign data loaded</p>
                <p className="text-xs text-gray-500 mt-1">
                  Go to Calibrate to record sign references first, or use demo mode
                </p>
              </div>
            </div>
          )}
