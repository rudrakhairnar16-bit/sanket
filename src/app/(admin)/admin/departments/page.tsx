...
export default function DepartmentsPage() {
  ...
  const reload = async () => { const r=await fetch("/api/admin/departments"); const d=await r.json(); if(d.success)setDepartments(d.departments); };
  const addDepartment = async () => { const name=window.prompt("Department name","Citizen Support Services")?.trim(); if(!name)return; const r=await fetch("/api/admin/departments",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name})}); const d=await r.json(); if(!r.ok||!d.success){alert(d.error||"Could not create department");return;} await reload(); };
  const editDepartment = async (dept: Department) => { const name=window.prompt("Rename department",dept.name)?.trim(); if(!name||name===dept.name)return; const r=await fetch("/api/admin/departments",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:dept.id,name})}); const d=await r.json(); if(!r.ok||!d.success){alert(d.error||"Could not update department");return;} await reload(); };
  const deleteDepartment = async (dept: Department) => { if(!window.confirm(`Delete ${dept.name}? This removes only the department record.`))return; const r=await fetch(`/api/admin/departments?id=${encodeURIComponent(dept.id)}`,{method:"DELETE"}); const d=await r.json(); if(!r.ok||!d.success){alert(d.error||"Could not delete department");return;} await reload(); };
    ...
    <AppShell>
      <div className="page-container">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          ...
          <Button variant="primary" icon={<span>+</span>} onClick={addDepartment}>
            Add Department
          </Button>
        </div>
            ...
            {departments.map((dept) => (
              <Card key={dept.id} variant="spatial" hover className="group">
                ...
                <div className="flex gap-2">
                  <Button variant="secondary" size="sm" className="flex-1" onClick={() => router.push(`/admin/analytics?department=${encodeURIComponent(dept.name)}`)}>Analytics</Button>
                  <Button variant="ghost" size="sm" className="flex-1" onClick={() => editDepartment(dept)}>Edit</Button>
                  <Button variant="ghost" size="sm" className="text-red-400" onClick={() => deleteDepartment(dept)}>Delete</Button>
                </div>
              </Card>
            ))}
          </div>
