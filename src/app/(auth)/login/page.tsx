...
const demoUsers = [
  ...
  { username:"neha",name:"Neha Joshi",role:"Clerk",department:"Grievance & Complaints",avatar:"👩‍🔬",color:"green" },
  { username:"arjun",name:"Arjun Mehta",role:"Clerk",department:"Revenue Services",avatar:"👨‍💼",color:"gold" },
  { username:"pooja",name:"Pooja Nair",role:"Clerk",department:"Public Health Services",avatar:"👩‍⚕️",color:"teal" },
  { username:"rahul",name:"Rahul Verma",role:"Clerk",department:"Education Services",avatar:"👨‍🏫",color:"blue" },
  { username:"kavya",name:"Kavya Rao",role:"Clerk",department:"Social Welfare",avatar:"👩‍💼",color:"green" },
  { username:"imran",name:"Imran Khan",role:"Clerk",department:"General Citizen Services",avatar:"👨‍💼",color:"gold" },
  { username:"meena",name:"Meena Das",role:"Clerk",department:"Transport Assistance",avatar:"👩‍💼",color:"teal" },
  { username:"vikas",name:"Vikas Singh",role:"Clerk",department:"Electricity & Utility Assistance",avatar:"👨‍🔧",color:"blue" },
  { username:"anita",name:"Anita Iyer",role:"Clerk",department:"Public Information & Applications",avatar:"👩‍💼",color:"green" },
  { username:"wateradmin",name:"Water Department Admin",role:"Dept Admin",department:"Water Services",avatar:"💧",color:"gold" },
  { username:"orgadmin",name:"Municipal Operations Admin",role:"Org Admin",department:"All Departments",avatar:"🏢",color:"blue" },
  { username:"stateadmin",name:"Gujarat Accessibility Admin",role:"State Admin",department:"Gujarat",avatar:"🇮🇳",color:"teal" },
  { username:"nationaladmin",name:"National Accessibility Admin",role:"National Admin",department:"India",avatar:"🇮🇳",color:"green" },
  { username:"admin",name:"System Administrator",role:"Super Admin",department:"All Departments",avatar:"👩‍💻",color:"red" },
  { username:"interpreter",name:"Anjali Deshmukh",role:"Interpreter",department:"Interpreter Services",avatar:"🤝",color:"purple" },
];
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        if (result.success) {
          const known = demoUsers.find((d) => d.username === username);
          const roleMap: Record<string,string> = { admin:"super_admin", wateradmin:"dept_admin", orgadmin:"org_admin", stateadmin:"state_admin", nationaladmin:"national_admin", interpreter:"interpreter" };
          router.push(roleHome(roleMap[username] || (known?.role === "Clerk" ? "clerk" : undefined)));
        } else {
  ...
  const roleHome = (role?: string) => {
    ...
    if (["dept_admin", "org_admin", "state_admin", "national_admin", "super_admin"].includes(role || "")) return "/admin";
    return "/dashboard";
  };
  ...
  const demoLogin = async (demoUsername: string) => {
    ...
    if (result.success) {
      const roleMap: Record<string,string> = { admin:"super_admin", wateradmin:"dept_admin", orgadmin:"org_admin", stateadmin:"state_admin", nationaladmin:"national_admin", interpreter:"interpreter" };
      router.push(roleHome(roleMap[demoUsername] || "clerk"));
    } else {
